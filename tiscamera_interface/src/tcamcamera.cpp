...
TcamCamera::initialize_format_list()
{
    ...
    {
        VideoFormatCaps fmt = {}; // {} means zero initialisation since C++11
        ...
        if (s)
        {
            ...
            {
                for (uint i=0; i < gst_value_list_get_size(format); i++)
                {
            ...
            {
                for (uint i=0; i < gst_value_list_get_size(framerates); i++)
                {
